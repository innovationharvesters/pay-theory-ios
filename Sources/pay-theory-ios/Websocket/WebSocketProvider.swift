...
public class WebSocketProvider: NSObject {
    ...
    func startSocket(
        ...
        socketHandler: WebSocketProtocol
    ) async throws {
        log.info("WebSocketProvider::startSocket")

        return try await withCheckedThrowingContinuation { continuation in
            let urlSession = URLSession(
                configuration: .default, delegate: listener,
                delegateQueue: OperationQueue())
            let socketUrl =
                "wss://\(environment).secure.socket.\(stage).com/\(environment)/?pt_token=\(ptToken)"
    ...
    func sendMessageAndWaitForResponse(message: URLSessionWebSocketTask.Message)
        async throws -> String
    {
        return try await withCheckedThrowingContinuation { continuation in
    ...
    func sendMessage(
        message: URLSessionWebSocketTask.Message, handler: WebSocketProtocol
    ) {
        log.info("WebSocketProvider::sendMessage")

        if self.asyncResponseHandler != nil {
            log.error(
                "WebSocketProvider::Cannot send message while waiting for response"
            )
            return
        }
        webSocket?.send(
