...
let log = Logger(logHandlers: [
    createOSLogHandler(category: Logger.LOG_CATEGORY)
])

typealias LoggerTimerFormatterBlock = (_ duration: Double) -> String

/// Native iOS logging class for Pay Theory, with options to direct logs
/// to different destinations.
class Logger {
    static let LOG_SUBSYSTEM = "paytheory"
    static let LOG_CATEGORY = "General"
    ...
    required init(logHandlers: [LogHandler]) {
     ...
     */
    func stacktrace(
        type: LogType = .stacktrace, file: String = #fileID, line: UInt = #line
    ) {
        guard type.rawValue >= minLevel.rawValue else {
            return
        }
        let queueName =
            OperationQueue.current?.underlyingQueue?.label ?? "<unknown>"
        ...
        log(
            type: type,
            "The stacktrace from '\(file):\(line)' on queue '\(queueName)':")
     ...
     */
    func callAsFunction(_ items: Any...) {
        log(type: .debug, items)
    }
     ...
     */
    func startTimer(_ formatterBlock: @escaping LoggerTimerFormatterBlock)
        -> LoggerTimer
    {
...
private func reformatStackSymbol(_ symbol: String) -> String {
    return symbol.replacingOccurrences(
